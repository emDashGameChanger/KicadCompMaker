...
## Status
🟢 active — Through Hole (resistors, aluminum electrolytic caps, disc/film caps) is feature-complete at 1.0, including batch import. Surface Mount and Diodes are still placeholder-only tabs in the GUI.
...
Also just wired the project into the OS: `plugin.py` is symlinked to `~/.local/bin/kicadcompmaker` and there's now a `~/.local/share/applications/kicadcompmaker.desktop` launcher (Categories: Science;Electronics) so it can run standalone outside KiCad via the `if __name__ == "__main__"` entry point at the bottom of `plugin.py` (see [Architecture](#architecture) below on why that path exists).
